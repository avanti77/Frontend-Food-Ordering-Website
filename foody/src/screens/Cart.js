...
export default function Cart() {
  ...
  <div className='container m-auto mt-5 table-responsive table-responsive-sm table-responsive-md'>
      ...
      <tbody>
        {data.map((food, index) => (
          <tr key={index}>
            ...
            <td style={{ color: 'blue' }}>{food.name}</td>
            <td style={{ color: 'green' }}>{food.qty}</td>
            <td style={{ color: 'orange' }}>{food.size}</td>
            <td style={{ color: 'purple' }}>{food.price}</td>
            <td>
              <button type="button" className="btn p-0" style={{ color: 'red' }}>
    ...
    <div><h1 className='fs-2' style={{ color: 'brown' }}>Total Price: {totalPrice}/-</h1></div>
    <div>
